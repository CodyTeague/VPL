...
import { mailApp, sheetApp } from './gscripts';
...
function getSpreadsheetData(sheetName) {
    ...
    var arrayOfArrays = SpreadsheetApp.openById(id).getSheetByName(sheetName).getDataRange().getValues();
...
function testPostRankings() {
    var teamsHere = [];
    var teamOne = { team: 1, wins: 0, losses: 1 };
    var teamTwo = { team: 2, wins: 1, losses: 0 };
    var teamThree = { team: 3, wins: 1, losses: 0 };
    teamOne['win_pct'] = getWinPct(teamOne);
    teamTwo['win_pct'] = getWinPct(teamTwo);
    teamTwo['win_pct'] = getWinPct(teamTwo);
    teams.push(teamOne);
    teams.push(teamTwo);
    teams.push(teamThree);
    postRankings(teamsHere);
}
...
function gamesBehind(teamA, teamB) {
...
function postRankings(records, orange) {
    ...
    var ss = SpreadsheetApp.openById(id).getSheetByName(board);
    var data = [];
...
function testGetWinPct(){
  var team = {'wins': 1, 'losses': 1}
  var winPct = getWinPct(team)
}
...
function recordScore(gameNumber, aWinsAll, aOne, bOne, aWinsOne, aTwo, bTwo, aWinsTwo, aTre, bTre, aWinsTre, name) {    
    var ss = SpreadsheetApp.openById(id).getSheetByName(backingData);
    var data = getSpreadsheetData(backingData);
    var row = 0;
    while (row < data.length) {
